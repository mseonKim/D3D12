...
void PyramidApp::Draw(const GameTimer& gt)
{
    ...
    SubmeshGeometry submesh = mPyramidGeo->DrawArgs["pyramid"];
...
void PyramidApp::BuildBoxGeometry()
{
    std::array<Vertex, 5> vertices =
    {
        ...
        Vertex({ XMFLOAT3(+1.0f, +1.0f, -1.0f), XMFLOAT4(Colors::Green) }),
        Vertex({ XMFLOAT3(+1.0f, -1.0f, -1.0f), XMFLOAT4(Colors::Green) }),
        Vertex({ XMFLOAT3(0.0f, 0.0f, +1.0f), XMFLOAT4(Colors::Red) }),
    };
    ...
    mPyramidGeo->VertexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(), mCommandList.Get(),
        vertices.data(), vbByteSize, mPyramidGeo->VertexBufferUploader);
